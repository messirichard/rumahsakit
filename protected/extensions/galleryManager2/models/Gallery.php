...
class Gallery extends CActiveRecord
{
    ...
    public function rules()
    {
        ...
        return array(
            array('name, description, title, content, active', 'safe'),
            // The following rule is used by search().
            // Please remove those attributes that should not be searched.
            array('id, sizes, name, description, title, insert_by', 'safe', 'on' => 'search'),
        );
    ...
    public function relations()
    {
        ...
        return array(
            ...
            'gallery' => array(self::HAS_ONE, 'GalleryPhoto', 'gallery_id', 'order' => '`rank` asc'),
        );
    }
    ...
    {
        $criteria = new CDbCriteria;
        $criteria->select = 't.*, YEAR(date_input) as `year`, MONTH(date_input) as `month`';
        $criteria->addCondition('status = 1');
        ...
        foreach ($gallery as $key => $value) {
            $data[$value->year][$value->month][$key] = $value;
        }
        return $data;
    }
